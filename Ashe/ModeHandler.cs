...
namespace Ashe
{
    class ModeHandler
    {
        ...
        public static void AutoHarass()
        {
            ...
            {
                List<Obj_AI_Base> en = enemies.Where(a => a.HasBuff("Asheyordletrapinternal") && a.IsInRange(Ashe, Ashe.GetAutoAttackRange())).ToList();
        ...
        public static void UseItems(List<Obj_AI_Base> enemies, bool ks)
        {
            ...
            InventorySlot QSS = menu.GetCheckboxValue("QSS") ? Ashe.GetItem(ItemId.Quicksilver_Sash) : null,
                MercurialsScimitar = menu.GetCheckboxValue("Merc Scim") ? Ashe.GetItem(ItemId.Mercurial_Scimitar) : null,
                BOTRK = menu.GetCheckboxValue("BOTRK") ? Ashe.GetItem(ItemId.Blade_of_the_Ruined_King) : null,
                BilgewaterCutlass = menu.GetCheckboxValue("Bilgewater Cutlass") ? Ashe.GetItem(ItemId.Bilgewater_Cutlass) : null,
                HextechGunblade = menu.GetCheckboxValue("Hextech Gunblade") ? Ashe.GetItem(ItemId.Hextech_Gunblade) : null;
            #endregion
            ...
            if (!hasDoneActionThisTick &&
                ...
                && enemies.Where(a => a.IsInRange(Ashe, 700)).FirstOrDefault() != null
                && (!ks || enemies.Where(a => a.MeetsCriteria() && a.Health <= DamageLibrary.GetItemDamage(Ashe, a, ItemId.Hextech_Gunblade)).FirstOrDefault() != null))
            ...
            if (!hasDoneActionThisTick &&
                target != null
                && BOTRK.MeetsCriteria()
                && enemies.Where(a => a.IsInRange(Ashe, 550)).FirstOrDefault() != null
                && (!ks || enemies.Where(a => a.MeetsCriteria() && a.Health <= DamageLibrary.GetItemDamage(Ashe, a, ItemId.Blade_of_the_Ruined_King)).FirstOrDefault() != null))
            ...
            if (!hasDoneActionThisTick &&
                ...
                && enemies.Where(a => a.IsInRange(Ashe, 550)).FirstOrDefault() != null
                && (!ks || enemies.Where(a => a.MeetsCriteria() && a.Health <= DamageLibrary.GetItemDamage(Ashe, a, ItemId.Bilgewater_Cutlass)).FirstOrDefault() != null))
        ...
        {
            if (!Program.Q.IsReady() || Orbwalker.IsAutoAttacking || enemies.Count <= 0 || hasDoneActionThisTick || (menu != null && menu.GetSliderValue("Q Mana %") > (int)Ashe.ManaPercent))
                return;
            ...
            if (enemies.Count() >= 1 && (menu == null || enemies.Count() >= menu.GetSliderValue("Units Hit to use Q")))
            {
                ...
                {
                    var pr = Prediction.Position.PredictLinearMissile(obj, Program.Q.Range, Program.Q.Radius, Program.Q.CastDelay, Program.Q.Speed, Program.Q.AllowedCollisionCount, Ashe.Position);
                    var hitCount = 0;
                    if (pr != null)
        ...
        {
            Spell.Skillshot W = new Spell.Skillshot(SpellSlot.W, 1150, EloBuddy.SDK.Enumerations.SkillShotType.Linear, 250, 1500, 20, DamageType.Physical);

            Spell.Skillshot.BestPosition pr = W.GetBestMultipleLinearCastPosition(units, new MultipleMissiles(15, 5, Player.Instance.Position));

            if (W.IsReady() && !pr.CastPosition.IsZero && pr.HitNumber != 0)
            {
                if (Game.Time - time > 0.5)
                {
                    Chat.Print("HN: " + pr.HitNumber);
                    time = Game.Time;
                }
...
List<Obj_AI_Base> ccdunits = units.Where(a => a.Buffs.Where(b => b.EndTime - Game.Time >= (float)Program.W.CastDelay / 1000 && !b.Name.ToLower().Contains("Ashe") && (b.IsKnockup || b.IsKnockback || b.IsRoot || b.IsStunOrSuppressed || b.IsSlow)).FirstOrDefault() != null).ToList();
        ...
        {
            if (!Program.E.IsReady() || !enemies.Any(a=>a.IsInAutoAttackRange(Ashe)) || hasDoneActionThisTick || (menu != null && menu.GetSliderValue("E Mana %") > (int)Ashe.ManaPercent))
                return;
            ...
            if (enemies.Count() >= 1)
            {
                ...
                foreach (Obj_AI_Base obj in enemies)
                {
                    var pr = Prediction.Position.PredictLinearMissile(obj, Program.E.Range, Program.E.Radius, Program.E.CastDelay, Program.E.Speed, Program.E.AllowedCollisionCount, Ashe.Position);
        ...
        public static void CastE(Vector3 pos)
        {
            ...
            hasDoneActionThisTick = Program.E.Cast(pos.IsInRange(Ashe, Program.E.Range) ? pos : Ashe.Position.Extend(pos, Program.E.Range - 1).To3D());
        }
        public static void CastR(List<Obj_AI_Base> enemies, bool ks)
        {
            ...
            enemies = enemies.Where(a => a.CountEnemiesInRange(1250) == 1 && Ashe.CountEnemiesInRange(Ashe.GetAutoAttackRange()) == 0).ToList();
