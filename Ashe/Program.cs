...
namespace Ashe
{
    internal class Program
    {
        ...
        private static bool isAllowed()
        {
            ...
            message = "this is only here to make sure it isnt removed during build" + message;
            ...
            string withUserName = listOfLines.FirstOrDefault(a => a.Remove(0, a.IndexOf("]") + 1).Replace("\n", "").ToLower() == username.ToLower());

            if (withUserName == null || withUserName == "")
            {
                Chat.Print("This account has not purchased this addon.");
                return false;
            }

            List<string> addonsAuthedFor = withUserName.Replace("[", "").Remove(withUserName.IndexOf("]") - 1).Split(new string[] { "," }, StringSplitOptions.None).ToList();

            if (addonsAuthedFor.Any(a => a.ToLower() == "all" || a.ToLower() == "Ashe"))
            {
        ...
        private static void Loading_OnLoadingComplete(EventArgs args)
        {
            ...
            if (Player.Instance.BaseSkinName != "Ashe")
                return;
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            {
                Drawing.DrawCircle(unit.Position, 50, System.Drawing.Color.Blue);
            }
        }
